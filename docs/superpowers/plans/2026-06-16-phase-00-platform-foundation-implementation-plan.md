...
- Best-effort organization settings audit rows in `audit_event`.
- Currency seed migration for `INR`, `USD`, `EUR`, and `GBP`.
...
sequenceDiagram
  ...
  API->>Tables: Fire-and-forget user audit_event
...
best-effort audit outside the response path; future posting commands
...
export function buildOrganizationSettingAuditRow(input: {
  ...
  // Builds best-effort audit metadata. No outbox row is emitted until a real
...
The oRPC procedure validates the shared settings input, verifies organization
membership through `organizationProcedure`, writes `organization_setting`, emits
best-effort audit without awaiting it, and returns `{ ok: true,
organizationId }`.
...
- Sensitive settings mutation writes `organization_setting` and emits best-effort `audit_event`; future accounting-critical commands write awaited audit/outbox rows in the same transaction.
