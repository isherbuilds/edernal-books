...
**Goal:** Establish organization-scoped platform foundation for the accounting app: auth, business settings, request context, tenant-scoped DB queries, audit events, outbox events, and idempotency ledger.
...
- `organization_setting`, `currency`, `audit_event`, `outbox_event`, and `idempotency_ledger`.
...
- Best-effort organization settings activity rows in `audit_event`.

Remaining Phase 0 work before Phase 1:

- Apply the migration in local development.
- Add currency seed data.
- Add role permission helpers and tests.
- Build onboarding/business settings UI.
- Add idempotency helper before create/post commands.
- Add tests around organization membership checks and settings writes.
...
sequenceDiagram
  ...
  API->>Tables: Fire-and-forget user activity audit_event
...
- Future `packages/db/src/queries/*`: reusable organization-scoped query helpers.
...
- `packages/api/src/context.ts`: session, organization membership, role, request metadata.
...
rollback together. Settings currently writes `organization_setting` and emits
best-effort audit activity outside the response path; future posting commands
must use awaited transactional audit/outbox when correctness depends on it.
...
- [x] **Step 6: Add `idempotency_ledger`**
...
Expected: Phase 0 app-owned tables are generated and migrated locally. Better Auth-owned tables remain generated by Better Auth CLI/plugin.
...
/**
 * Returns the verified business ID or fails before service code touches tenant data.
 *
 * Why this exists: API handlers should not rely on optional organization context
 * when reading or writing app-owned tenant tables.
 */
...
`packages/api/src/context.ts` reads the Better Auth session and request
metadata. Organization-scoped routes accept `orgId` or `orgSlug` input, verify
membership, then expose organization ID and role through `AppContext`.
...
The oRPC procedure validates `upsertOrganizationSettingInput`, calls `requireOrganization`, writes `organization_setting`, `audit_event`, and `outbox_event` inside one `db.transaction`, and returns the saved settings.
