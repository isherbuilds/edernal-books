...
type OrganizationPermission = (role: string) => boolean;

function requireOrganization(permission?: OrganizationPermission) {
  return o.middleware(async ({ context, next }, input: OrgSlugInput) => {
    ...
    if (permission && !permission(membership.role)) {
      throw new ORPCError("FORBIDDEN");
    }
...
export function organizationPermissionProcedure<TInput extends OrgSlugInput>(
  inputSchema: z.ZodType<TInput>,
  permission: OrganizationPermission
) {
  return publicProcedure
    .input(inputSchema)
    .use(requireOrganization(permission))
    .route(authCookieSecurity);
}
